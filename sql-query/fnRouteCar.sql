DROP FUNCTION if exists routeBetweenPointsCar;

--v2.6
CREATE FUNCTION routeBetweenPointsCar(
...
--    FROM points order by id asc limit 1;
    ...
    CREATE TEMP TABLE route ON COMMIT DROP AS (
        SELECT di.seq, routing_car.way, routing_car_vertices_pgr.the_geom as node FROM pgr_trsp(
                'SELECT id::integer, source::integer, target::integer, cost, reverse_cost, way FROM routing_car',
                source_id,
        ...
        LEFT JOIN routing_car 
        ON di.id2 = routing_car.id
        LEFT JOIN routing_car_vertices_pgr
        ON di.id1 = routing_car_vertices_pgr.id
    );
