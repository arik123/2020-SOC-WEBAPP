...
DROP FUNCTION if exists pgr_nodeNetworkMy;

--v2.6
CREATE FUNCTION pgr_nodeNetworkMy(
...
----------------
  ...
  p_ret = 'CREATE TEMP TABLE intergeom ON COMMIT DROP AS (
    ...
    st_equals(_pgr_endpoint(l1.' || quote_ident(n_geom) || '),_pgr_endpoint(l2.' || quote_ident(n_geom) || '))=false AND
    FLOOR(l1.z_order/100)=FLOOR(l2.z_order/100)
    )';
  RAISE DEBUG '%', p_ret;
  EXECUTE p_ret;
...
-- COMMENTS
COMMENT ON FUNCTION pgr_nodeNetworkMy(TEXT, DOUBLE PRECISION, TEXT, TEXT, TEXT, TEXT, BOOLEAN) IS 'pgr_nodeNetworkMy
- Parameters
  - Edge table name
  - tolerance
- Optional parameters
